...
{
  m_node.route("/READ", [&](const Interest& interest, const Name& args, shared_ptr<Data> data,
                            InterestShower show) {
  ...
  m_node.route("/IDENTITY/for", 
               std::bind(&Service::onGetIdentityKey, &m_service, _1, _2, _3, _4));

  m_node.route("/MANAGEMENT/identity/add",
               std::bind(&Service::onAddIdentity, &m_service, _1, _2, _3, _4));

  m_node.route("/MANAGEMENT/identity/remove",
               std::bind(&Service::onRemoveIdentity, &m_service, _1, _2, _3, _4));

  m_node.route("/MANAGEMENT/access/grant", 
               std::bind(&Service::onGrant, &m_service, _1, _2, _3, _4));

  m_node.route("/MANAGEMENT/access/revoke",
               std::bind(&Service::onRevoke, &m_service, _1, _2, _3, _4));
}
